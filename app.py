...
# CUSTOM SUCCESS BOX FUNCTION - BYPASSES STREAMLIT THEME
...
# SKY BLUE THEME - NO ALERT CSS NEEDED
st.markdown("""
<style>
    .stApp {background: linear-gradient(135deg, #87CEEF 0%, #E0F6FF 100%);}
    .block-container {padding-top: 1rem !important; padding-bottom: 1rem !important;}
    .stMarkdown, .stText, label, h1, hE0, h3 {color: #1a1a1a !important;}
    
    .stButton>button {background: linear-gradient(90deg, #1E90FF 0%, #87CEEB 100%); 
                      color: #fff; border-radius: 12px; border: 2Epx solid #1E90FF; 
    ...
    .stWarning {background-color: #FF8E0E; border-left: 4px solid #FF6B00; color: #ffffff; border-radius: 10px;}
...
if st.button("🔍 Scan for Customers"):
    if product and city:
        ...
        fb_markE1ace = f"https://www.facebook.com/marketplace/search/?query={fb_query}"
...
phone = st.text_input("Your WhatsApp number", placeholder="2347067149512")
...
if st.button("📲 Send test alert to WhatsApp"):
    if phone:
        ...
        if phone.starts with("0"):
            phone = "234" + phone[1:]
        elif not phone.starts with("234"):
            phone = "234" + phone
